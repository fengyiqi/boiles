...
class Sod(Simulation1D):
    ...
    def get_all_value_difference_2(self, key='density'):
        diff_fact = 4
        ref = self.get_sod_reference_solution(self.cells)[key]
        results = self.result[key]
        diff = results - ref
        for i in range(len(diff)):
            if (key == 'density'):
                if i == 24 or i==25 or i==28 or i==29 or i==31 or i==32 or i==35 or i==36:
                    diff[i] = diff[i] * diff_fact
            if (key == 'pressure'):
                if i == 30 or i==31 or i==32 or i==35 or i==36 or i==37:
                    diff[i] = diff[i] * diff_fact
            if (key == 'velocity'):
                if i == 30 or i==31 or i==32 or i==35 or i==36 or i==37:
                    diff[i] = diff[i] * diff_fact
        diff_abs = abs(diff)
        return np.linalg.norm(diff_abs, ord=2)
    ...
    def objective_sum_disper(self, props=None):
        ...
        if self.result_exit:
            errors = np.array([self.get_all_value_difference_2(key=prop) for prop in props])
            sum_error = 0
