...
OP.test_cases = [SodDisper60, SodShock60, TGVTENO5]
OP.ref_point = [SodDisper60.ref_point, SodShock60.ref_point, TGVTENO5.ref_point]
OC.num_inputs = 2
OC.num_outputs = 3
OC.initial_samples = 30
OC.opt_iterations = 30
OC.bounds = [(1e-6, 3.5e-1), (1e-4, 3.5e-1)]
SC.Riemann = "roe"
...
x1 = RangeParameter(name="sp", lower=OC.bounds[0][0], upper=OC.bounds[0][1], log_scale=False,
                    parameter_type=ParameterType.FLOAT)
x2 = RangeParameter(name="st", lower=OC.bounds[1][0], upper=OC.bounds[1][1], log_scale=False,
                    parameter_type=ParameterType.FLOAT)

search_space = SearchSpace(
    parameters=[x1, x2],
    parameter_constraints=[OrderConstraint(x1, x2)]
)
...
class MetricSodShock(NoisyFunctionMetric):
    def f(self, x: np.ndarray) -> float:
        _, error_shock = sod_tgv.sod_error(x)
        return error_shock
...
metric_sod_disper = MetricSodDisper("disper", ["sp", "st"], noise_sd=0.0, lower_is_better=True)
metric_sod_shock = MetricSodShock("shock", ["sp", "st"], noise_sd=0.0, lower_is_better=True)
metric_tgv = MetricTGV("tgv", ["sp", "st"], noise_sd=0.0, lower_is_better=True)
...
mo = MultiObjective(
    metrics=[metric_sod_disper, metric_sod_shock, metric_tgv],
)
